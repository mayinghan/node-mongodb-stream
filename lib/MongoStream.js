...
class MongoStream extends Event {
    constructor({ connection, db, collection, op }) {
        ...
        MongoClient.connect(connection.url).then(db => {
            let stream = op.name === 'aggregate' ? db(db).collection(collection).aggregate(op.pipeline, op.options)
                : db(db).collection(collection)[op.name](op.query, op.options).stream();
