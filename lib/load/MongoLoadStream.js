...
class MongoLoadStream extends Event {
    ...
    upsert(items) {
        const getOp = (bulk, item) => bulk.find(pick(this.key, item)).upsert().update({ $set: pick(this.udpateKeys, item), $setOnInsert: pick(this.insertKeys, item) }) && bulk;
        const batchUpsert = (op, items) => reduce(op, this.bulk, items).execute();
        const batchByItems =  batch => batchUpsert(getOp, batch);
